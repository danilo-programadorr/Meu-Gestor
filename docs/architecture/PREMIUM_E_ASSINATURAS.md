...
O SUB-1E-2 prevê `closedTestGrant` somente para o teste fechado: uma janela global UTC fixa de quinze dias, calculada no backend futuro para `development` e track `closed`, com as cinco capabilities Premium. Não é assinatura, compra, preço, oferta Play ou direito de production. O app não cria, restaura, reutiliza nem mede a janela por relógio próprio; ao expirar, o backend remove as capabilities e a experiência segue o fluxo comercial normal sem popup modal. Production só aceitará entitlement verificado da Google Play depois do lançamento oficial.
...
As callables futuras são verificação, restauração e leitura de entitlement confirmado. Todas exigirão App Check individualmente, sem enforcement global. RTDN e concessão fechada são caminhos HTTP/administrativos distintos: RTDN precisa de perímetro autenticado e reconsulta a fonte autoritativa; grant exige identidade administrativa server-side, lista autorizada e janela fixa. O aplicativo, owner e qualquer UID não obtêm escrita de entitlement ou grant.
