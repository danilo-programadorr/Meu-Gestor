...
## Nota SUB-1D/SUB-1E-1 — nenhum dado comercial novo no cliente

SUB-1D/SUB-1E-1/SUB-1E-2/SUB-1E-3A não alteram esquema publicado, índices, documentos reais ou Rules publicadas. O catálogo comercial aprovado — produto `meu_gestor_premium`, planos-base `mensal`/`anual` e oferta mensal `teste-3d` — não é documento Firestore nem concede acesso pelo cliente. Produto Google Play, preço localizado, token, recibo, identificador ofuscado, eventos de verificação, binding, acknowledgement, RTDN e auditoria administrativa continuarão fora de `users/{uid}/entitlements/premium` e serão exclusivos de backend futuro. A página Premium só lê o entitlement canônico existente.
...
O aplicativo poderá ler somente o entitlement do próprio UID e nunca poderá criar, ativar, renovar, revogar, reembolsar ou conceder. Purchase token, recibo bruto, payload do provedor, credenciais, dados de cartão, preço e auditoria confidencial não pertencerão ao documento legível pelo cliente. Grants administrativos e de development terão validade, ambiente e trilha de auditoria server-side; owner não recebe acesso cruzado. `closedTestGrant` é exclusivamente development/track fechado, recebe validade individual de 15 dias iniciada pelo servidor e nunca poderá ser aceito em production. O diretório interno `_premiumClosedTestTesters/{uid}` não contém e-mail e é negado integralmente ao cliente, assim como `_premiumClosedTestGrants`.
