...
| INV-203 | Preservar isolamento financeiro | P0 | INV-2A | não persiste entrada nem altera carteira, saldo, conta, lançamento, provento ou resumo | implementado localmente | domínio isolado | crítico | zero externo |
| INV-CALC-201 | Descobrir prazo ou aporte do primeiro milhão | P0 | INV-CALC-2 | usuário escolhe a incógnita; prazo desejado aceita anos e 0–11 meses; resultado detalha meta, prazo, aporte, capital colocado, rendimento matemático e saldo | implementado localmente | domínio inteiro determinístico | alto: evita interpretação ambígua | zero externo |
| INV-CALC-202 | Explicitar unidades e fórmulas | P0 | INV-CALC-2 | juros simples separam taxa anual/dias; compostos taxa mensal/meses; porcentagem, variação, Graham e Bazin têm campos e resultados próprios | implementado localmente | testes de domínio/widget | alto: reduz erro de entrada | zero externo |
| CRUD-001 | Cobrir ações de todas as entidades | P0 | CRUD-AUDIT-1 | matriz contém cadastrar, visualizar, editar, excluir, arquivar, cancelar, anular e restaurar, incluindo ausências justificadas | implementado localmente | teste estrutural e suítes funcionais | alto: evita ação indevida ou ausente | zero externo |
| CRUD-002 | Corrigir e excluir ativo vazio com segurança | P0 | CRUD-AUDIT-1 | schema 2 sem operação/provento permite nome/tipo e exclusão revalidada; histórico/legado permite correção de nome e arquivamento, nunca delete | implementado localmente | repositório, Rules locais e Emulator | crítico | até duas consultas server-only e transação |
| CRUD-003 | Padronizar ações acessíveis | P0 | CRUD-AUDIT-1 | ícones isolados usam componente canônico, tooltip, semântica por entidade, cor destrutiva e estado desabilitado; confirmações explicam consequências | implementado localmente | Material e testes de widget | médio | neutro |
...
| ASSIST-101 | Exibir consentimento antes da leitura | P0 | ASSIST-1A | escopo é visível; sem consentimento o componente não observa providers financeiros; escolha pode ser revista | implementado localmente | perfil confirmado | crítico | zero externo |
| ASSIST-102 | Responder perguntas guiadas sem IA | P0 | ASSIST-1A | mês, saldo, compromissos e investimentos usam cálculos determinísticos, sem texto livre, memória ou provedor | implementado localmente | fontes atuais | alto | zero externo |
| ASSIST-103 | Informar fontes, períodos e lacunas | P0 | ASSIST-1A | toda resposta informa data/período de São Paulo e fontes; leitura ausente não é estimada | implementado localmente | estados server-confirmed | crítico | zero externo |
| ASSIST-104 | Preservar privacidade e somente leitura | P0 | ASSIST-1A | privacidade oculta cifras/contagens; nenhuma recomendação, gravação ou execução é exposta; 320 px e fonte ampliada passam | implementado localmente | controle global de privacidade | crítico | zero externo |
| ASSIST-110 | Comparar privacidade e retenção oficiais | P0 | ASSIST-1B-0 | OpenAI, Gemini pago/Vertex e Anthropic têm treino, retenção padrão/reduzida e exceções documentados; free tier Gemini é recusado para contexto financeiro | auditoria documental local | decisão jurídica e comercial futura | crítico | zero externo |
...
| ASSIST-150 | Executar prova sintética sem dados de usuário | P0 | ASSIST-2A | endpoint global, oito chamadas máximas, quatro fixtures sintéticas por modelo e nenhum áudio, imagem, grounding, cache, ferramenta ou streaming | benchmark controlado em development | não integra aplicativo nem seleciona fornecedor | crítico | teto local de R$ 2,00 |
| ASSIST-151 | Reter somente métricas agregadas | P0 | ASSIST-2A | contrato aceita apenas modelo, rótulo sintético, latência, tokens, custo estimado e nota estrutural; prompt e resposta são recusados | implementado e testado localmente | política de retenção e backend futuros | crítico | zero adicional |
| ASSIST-152 | Falhar antes de exceder limites do benchmark | P0 | ASSIST-2A | quantidade, tokens, custo e campos são validados antes do registro; Pro truncado não é avaliado como baixa qualidade | implementado e testado localmente | novo ensaio exige autorização | alto | teto adicional encerrado |
| ASSIST-160 | Preparar contrato remoto sem exposição de contexto | P0 | ASSIST-2B-0 | Flutter envia somente mensagem e versão; identidade, consentimento, modelo, fatos e credenciais ficam fora do contrato | implementado localmente e desligado | Function protegida e consentimento de envio futuros | crítico | zero externo |
| ASSIST-161 | Manter kill switch e fail-closed antes da ativação | P0 | ASSIST-2B-0 | kill switch ativo, flag real compilada como falsa, roteador server-side e métrica sem conteúdo sensível | implementado e testado localmente | rollout development com aprovação separada | crítico | zero externo |
| ASSIST-170 | Reservar custo de IA sem reter conteúdo sensível | P0 | ASSIST-2C-CONTROL-STORE-1 | ledger transacional aceita somente requestId aleatório, tier, duração e custos inteiros; reserva prévia, idempotência e inconsistência falham fechadas | implementado e testado localmente | banco nomeado e runtime restrito em development | crítico | R$ 5/dia e R$ 45/mês internos |
| ASSIST-171 | Preparar callable remota sem ativar provedor | P0 | ASSIST-2D-0 | callable Gen 2 aceita somente versão e mensagem; revalida Auth, e-mail, App Check, perfil, consentimento e privacidade; retorna `safe_unavailable` com kill switch ativo e flag falsa | implementado e testado localmente | composição server-side e deploy aprovados separadamente | crítico | zero externo |
| ASSIST-172 | Registrar composição Gen 2 sem recurso em nuvem | P0 | ASSIST-2E-0 | export futuro `assistRemoteV1` fixa região, limite de instâncias, timeout, memória e App Check; não contém SDK Vertex, URL, projeto ou segredo | implementado e testado localmente | codebase, runtime identity e deploy aprovados separadamente | crítico | zero externo |
...
| ID | Requisito | Prioridade | Incremento | Critério de aceite | Situação atual | Dependências | Impacto de segurança | Impacto de custo |
|---|---|---:|---:|---|---|---|---|---|
| DATA-1A | Contrato de reset financeiro | P0 | DATA-1A/PRIV-1A | manifesto fechado, estados, cursor, idempotência, falhas sanitizadas e recibo anônimo definem o fluxo sem execução | domínio local e testes puros; preservado pelo DATA-1B | Function futura, repositórios e Emulator Suite | crítico | leituras/escritas a estimar |
| PRIV-1A | Contrato de exclusão da conta | P0 | DATA-1A/PRIV-1A | Auth recente, App Check, e-mail, perfil jurídico, UID próprio, frase exata e fluxo terminal são contratos do backend futuro | domínio local e testes puros; preservado pelo PRIV-1B; relacionado a AUT-007 | política LGPD, Functions, Auth e regras | crítico | Functions/armazenamento a estimar |
| DATA-1B | Backend local de reset financeiro | P0 | DATA-1B/PRIV-1B | manifesto fechado, lock financeiro, cursor por alvo/documento, lote idempotente e timeout recuperável preservam perfil, consentimentos, aparência, Premium e owner | backend ESM local/fakes/testes; sem Function/Rules/tela | Function futura, repositórios e Emulator Suite | crítico | leituras/escritas a estimar |
| PRIV-1B | Backend local de exclusão da conta | P0 | DATA-1B/PRIV-1B | Auth recente, App Check, e-mail, perfil jurídico, UID próprio, exclusão terminal, revogação antes de Auth e recibo anônimo de 30 dias | backend ESM local/fakes/testes; sem exclusão real; relacionado a AUT-007 | política LGPD, Functions, Auth e regras | crítico | Functions/armazenamento a estimar |
| PRIV-1C/1D | UX, reautenticação e lock local | P0 | PRIV-1C/PRIV-1D | frases exatas, reautenticação, token renovado, UI sem falso sucesso e Rules que negam caminhos privados/lock sem bypass owner | local e testado no Emulator; sem Function/publicação/limpeza real | Function, cache coordenado e Rules publicadas | crítico | leituras de lock a estimar |
| PRIV-1E-A | Borda privada Gen 2 e integração Flutter local | P0 | PRIV-1E-A | três callables fechadas derivam Auth/App Check/e-mail/perfil/UID; mutações exigem `auth_time` ≤ 5 min, frase é descartada e resposta não vem de cache | codebase Node 22, adaptadores injetáveis e testes locais; sem deploy, storage Admin conectado ou exclusão real | PRIV-1E-B, Auth/Functions Emulator e auditoria externa | crítico | custo ainda não medido |
